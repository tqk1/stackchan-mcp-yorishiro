...
## 9. 追記（同日夜）: Phase B 実機達成 & レイテンシ短縮（VAD 自動停止）

### 何が起きたか（時系列）

1. **電源オフ無効化ファームを flash**（22:04）— 前夜ビルド済みだったバイナリを app パーティションへ書き込み。「WS 切断 5 分で勝手に電源が切れる」問題はこれで根治。
2. **Phase B 完了条件を実機達成** — タップ→話す→Hermes の返答を VOICEVOX 音声で発話、をユーザー確認 ✅
3. **遅延問題の調査** — 体感 10〜30 秒の正体はログで判明: 録音が毎回きっかり 498 フレーム（=30 秒のタイムアウト上限）まで続いていた。処理自体（STT+Hermes+TTS 合成）は 6〜9 秒。
4. **VAD 無音自動停止を実装**（コミット 682c38f）— 話し終わって 1.2 秒で録音が自動終了するようにした。
5. **実機テスト合格**（23:03）— 録音 2〜3 秒で自動停止、ターン全体 ~10 秒。ユーザー所感「とても流暢」。

### 学び 1: 遅延の犯人は「処理の遅さ」ではなく「録音の止めどき」だった

最適化というと推論の高速化を考えがちだが、今回はパイプラインの一番手前
（録音をいつ打ち切るか）がボトルネックだった。ログの実測値
（`timings_ms={'stt': ..., 'hermes': ..., 'tts': ...}`）を見たことで
思い込みを排除できた。**測ってから直す**の好例。

### 学び 2: STT の転写崩壊も同じ根本原因だった

「ん」「オーブン」のような意味不明な転写が続いていたが、マイクの故障では
なく **30 秒録音の大半を占める無音・環境ノイズが whisper を混乱させていた**。
録音が発話分だけ（2〜3 秒）になった途端、「お元気ですか」を正確に転写。
1 つの根本原因（録音が長すぎる）が 2 つの症状（遅延・転写崩壊)を
生んでいた。

### 学び 3: VAD とは / 今回の実装

- **VAD (Voice Activity Detection)** = 音声区間検出。「いま人が喋っているか」を
  判定する技術。ESP32 側の AFE (Audio Front End, espressif の音声前処理
  ライブラリ) に内蔵されており、もともと LED 表示にだけ使われていた。
- 実装は `firmware/main/boards/stackchan/stackchan.cc` の `PollTouchpad()`
  （50ms 周期ループ）に追加。`Application::IsVoiceDetected()` をポーリングし:
  - listen 開始直後 800ms は判定保留（起動音の自己拾音を VAD が誤検知するため）
  - 発話を一度でも検知した後、無音が 1200ms 続いたら `StopListening()`
  - **発話を一度も検知していなければ止めない** — VAD が拾えないほど声が
    小さい場合にユーザーの発話を切り捨てないための保険。30 秒タイムアウトと
    再タップ送信が引き続き backstop として機能する
- 22:56 のテストで一度「30 秒待ち」が再現したのはこの保険が働いたケース
  （声が小さい/遠いと VAD が検知しない）。近くではっきり話せば 2〜3 秒で切れる。

### 現在のターン内訳（実測、ウォーム時）

| 区間 | 時間 |
|---|---|
| 録音（発話 + 無音 1.2s） | 発話長 + 1.2s |
| STT (faster-whisper, CPU int8) | 0.6s |
| Hermes 思考 | 2.6〜3.4s |
| VOICEVOX 合成 | 2〜4s |
| 音声再生（リアルタイム送出） | 返答の長さ分 |

発話終了 → 返事が鳴り始めるまで: **約 5〜7 秒**

### おまけ: 診断用の録音ダンプ機能（コミット 8c59987）

マイク劣化を疑った際に、gateway の `/voice_turn` に受信音声をファイル保存する
機能を追加した（環境変数 `STACKCHAN_VOICE_DUMP_DIR` を設定したときだけ有効、
通常は no-op）。結局マイクは正常だったが、今後の音声トラブル調査に使える。

### 残タスク（次セッション）

- STACKCHAN_TOKEN の設定（firmware ⇔ gateway の認証、B8 の残り）
